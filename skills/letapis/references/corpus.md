...
| `parser` | the reading this folder named for its files, `null` when it named none — see below. For a C++ project it decides whether the `.h` files are read as C++ |
...
### How a folder's files are read

A folder can name the reading its files get, and `parser` is where that is set:

```python
mcp__<engine>__update_folder(path="/path/to/cpp-project", parser="cpp")
```

**The value is a name the engine has registered, and the engine lists them.** `list_folders`
carries them under `parsers.lenses` and `parsers.cut`, so read the names there rather than from a
page. A name the engine does not have is refused with `unknown_parser` before anything is written,
and the refusal carries `known_parsers`. The value can also be a map of extension to name,
`{".xml": "<one>", ".html": "<another>"}`, for a folder that wants a different reading per
extension; each cell is checked on its own, and a refusal says which extension it was for. The
folder's own choice is the `parser` field of its row.

**A C++ project names `cpp` for the sake of its headers.** `.cpp`, `.cc`, `.cxx`, `.hpp`, `.hh`,
`.hxx`, `.cu` and `.cuh` are read as C++ in any folder, with a lens or without one. `.h` is not,
because a header can be plain C just as well as C++, and without the lens it is read as C. On a C++
header that reading drops every method whose body sits inside a class, together with the calls made
from those bodies, and says nothing about it. An out-of-class `Klass::method` in the same file can
still come back, so the answer looks complete while part of the file is missing. With `parser="cpp"`
the folder's `.h` files are read as C++.

**When the change takes effect depends on which list the name is in.** A name under
`parsers.lenses` decides what the call map reads, and `blast_radius` reads the files from disk each
time it is asked, so the new reading applies on the next lookup. A name under `parsers.cut` decides
how files are cut into chunks for search, and what is already indexed was cut under the old
setting: follow the change with `force_reindex` on the folder, or search by meaning goes on
answering from the old chunks and gives no sign of it. `cpp` is a lens only and needs no reindex; a
name that sits in both lists changes both.
