...
| `unparsed` | files that were opened and whose reader gave up: `{extension, files, with_the_name}`. Different from `unread`: there the language is unknown, here the file is. `with_the_name` counts the files that write the name itself — a file can be opened for a name it never writes, when a framework reading looks for a related one — and only those are said to carry the name. Such a file is also counted under `nothing_taken` in `narrowed_by`, and `symbol_found_on_disk` is `true` |
